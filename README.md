...
## 간략 소개
Flutter 기반 실서비스를 설계·운영·배포까지 경험한 개발자

Flutter를 중심으로 모바일, 웹, 하이브리드 앱까지 실제 서비스 개발과 운영을 경험해왔습니다.
소규모 팀 환경에서 기획 단계부터 개발, 배포, 유지보수까지 전 과정을 맡아왔고, 특히 기존 서비스의 유지보수와 구조 개선 경험이 많습니다.

최근에는 CRM 시스템에서 Flutter 기반 UX/UI를 담당하며, GetX 구조 개선, JWT 인증 전환, 일정·알림·검색 등 운영에 필요한 기능들을 구현했습니다.
이전에는 PHP 기반 웹앱을 Flutter 네이티브 앱으로 전환하는 프로젝트에 참여해 레거시 분석, 데이터 마이그레이션, API 구현까지 경험했습니다.

또한 Flutter로 하이브리드 앱을 단독 개발해 Google Play Store와 App Store에 직접 출시한 경험이 있으며, Firebase를 활용한 푸시 알림과 광고 연동, 스토어 심사 대응까지 전반적인 배포 과정을 경험했습니다.

기존에는 GetX 기반 MVVM 구조로 빠른 개발에 집중했다면, 최근에는 Riverpod와 DDD 구조를 학습·적용하며 유지보수성과 확장성을 고려한 아키텍처 설계에 집중하고 있습니다.
새로운 환경에서도 빠르게 코드베이스를 이해하고 실질적인 기여를 할 수 있는 Flutter 개발자라고 생각합니다.
...
- 연계/소속회사 : ㈜ 에이치아이티
- 주요 업무 : CRM시스템 유지보수 및 신규 기능 구현
- 담당 역할 : Flutter UX/UI
- 기술 스택 : FLUTTER, Next.js, TypeScript, AWS, MySQL, GitHub, Jira, Notion
- 업무 기간 : 2025.04 ~ 
- 개발 인원 : 3명
- 상세 내용 :
    - 라우팅 방법을 Get.to 방식에서 url을 이용한 Get.toNamed로 변경
    - 로그인 : Firebase를 활용한 로그인 프로세스에서 Jwt을 통한 로그인 프로세스로의 변경
    - 대시보드 : 해당 지점의 전반적 실황 보고
    - 일정/홀일정 : 기존 예약 일정과는 다르게 임시로 홀딩 할 수 있는 기능 추가, 필터
    - 멘션태그 : 메모 추가시 원하는 직원에게 멘션태그 걸 수 있도록 설정
    - 소셜 모니터 : 크롤링을 통한 해당 지점에 대한 게시물 데이터 수집, 감정분석
    - 빠른검색 : 고객명과 연락처로만 검색 가능한 간단한 검색 로직
    - 양식설정 : iframe을 이용한 webview형식으로 엑실을 띄워 실제 해당 셀을 클릭하여 수정할 수 있게 개선
    - 지점설정 : 현재 날짜와 예상 예식날짜 기준으로 급한 상담건을 표시한 기준 변경 로직, 보고 기준 설정에 따른 사내 디스코드 알림 전송
    - 메신저설정 : 기존의 급한 상담건 알림 외에도 모든 상담카드건에 대한 알림을 보낼 수 있도록 웹훅 추가
